...
struct ChannelItem: Identifiable, Hashable {
    ...
    var createrName: String {
        return members.first(where: { $0.uid == createdBy })?.username ?? "Unknown"
    }
    ...
    static let placeholder = ChannelItem.init(id: "1", lastMessage: "Hello World", creationDate: Date(), lastMessageTimeStamp: Date(), membersCount: 2, adminUids: [], membersUids: [], members: [], createdBy: "Max Mustermann")
...
extension ChannelItem {
    init(_ dict: [String: Any]) {
        ...
        self.membersUids = dict[.memberUids] as? [String] ?? []
